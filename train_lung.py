...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str,
                        default='/media/Storage2/zbw/data/covid19/Dataset/',
                        help='path to train dataset')

    opt = parser.parse_args()

    train_save = './model_pth/Lung/' + opt.model_name + '/'
    logging.basicConfig(filename='./logs/Lung/train_log_' + opt.model_name + '.log',
                        format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]',
                        level=logging.INFO, filemode='a', datefmt='%Y-%m-%d %I:%M:%S %p')
    print('batch_size:', str(opt.batchsize))

    for i in range(3):
        train_save = './model_pth/Lung/' + str(i) + '/' + opt.model_name + '/'
