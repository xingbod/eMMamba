...
class connect_loss(nn.Module):
    ...
    def multi_class_forward(self, c_map, target):
        ...
        onehotmask = F.one_hot(target.long(), 1)  # change it to your class number if needed
        onehotmask = onehotmask.permute(0, 3, 1, 2)
        onehotmask = onehotmask.float()

        ### get your connectivity_mask ###
        con_target = connectivity_matrix(onehotmask, 1)  # (B, 8*C, H, W)
        ...
        ### get edges gt###
        class_conn = con_target.view([c_map.shape[0], 1, 8, c_map.shape[2], c_map.shape[3]])
    ...
    def single_class_forward(self, c_map, target):
        ...
        ### get your connectivity_mask ###
        con_target = connectivity_matrix(target, 1)  # (B, 8, H, W)
